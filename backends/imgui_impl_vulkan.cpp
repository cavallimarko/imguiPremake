...
void main()
{
    // Pass-through: UNORM swapchain (Engine VulkanSwapchain) -- no hardware sRGB encode; pow() made UI too dark.
    Out.Color = aColor;
    Out.UV = aUV;
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (desc_set == VK_NULL_HANDLE)
                    continue; // Skip draw: texture was not properly uploaded (TexID is 0)
